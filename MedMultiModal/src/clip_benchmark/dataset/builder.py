...
from torchvision.datasets import (CIFAR10, CIFAR100, DTD, GTSRB, MNIST,
...
from .medical_datasets import BACH, LC25000, NCK_CRC, PCAM, RoCo, SicapDataset, MIMIC_CXR, MIMIC_CXR_LP, VinDr_Mammo, SkinCancer, MedMNIST, DeepEyeNet, Quilt1M, MedMNISTPlus, PadUfes20
...
def build_dataset(dataset_name, root="root", transform=None, split="test", download=True, annotation_file=None, language="en", task="zeroshot_classification", wds_cache_dir=None, custom_classname_file=None, custom_template_file=None, **kwargs):
    ...
    if dataset_name == "cifar10":
        ...
        from clip_benchmark.dataset import multilingual_mscoco
        ...
        from clip_benchmark.dataset import crossmodal3600
        ...
        from clip_benchmark.dataset import xtd200
        ...
        from clip_benchmark.dataset import flickr30k_200
        ...
        assert split in ("train", "val", "test"), f"Only `train` and `val` and `test` split available for {dataset_name}"
        ds = PCAM(dataset_name, root=root, split=split, transform=transform, **kwargs)
        ds.classes = default_classnames["pcam"]
    elif dataset_name == "nck":
        ds = NCK_CRC(dataset_name, root=root, split=split, transform=transform, **kwargs)
        ds.classes = default_classnames["nck"]
    elif dataset_name == "bach":
        ds = BACH(dataset_name, root=root, split=split, transform=transform, **kwargs)
        ds.classes = default_classnames["bach"]
    elif dataset_name == "lc25000_lung":
        ds = LC25000(dataset_name, root=root, split=split, transform=transform, organ="lung", **kwargs)
        ds.classes = default_classnames["lc25000_lung"]
    elif dataset_name == "lc25000_colon":
        ds = LC25000(dataset_name, root=root, split=split, transform=transform, organ="colon", **kwargs)
        ds.classes = default_classnames["lc25000_colon"]
    elif dataset_name == "sicap":
        if not os.path.exists(root):
            # Automatic download
            print("Downloading sicap-v2...")
            call(f"mkdir {root}", shell=True)
            call(f"cd {root} ; wget https://prod-dcd-datasets-cache-zipfiles.s3.eu-west-1.amazonaws.com/9xxm58dvs3-1.zip", shell=True)
            call(f"cd {root} ; unzip 9xxm58dvs3-1.zip ; rm 9xxm58dvs3-1.zip", shell=True)
            call(f"cd {root} ; unzip SICAPv2.zip ; rm SICAPv2.zip", shell=True)
            call(f"cd {root} ; mv SICAPv2/* ./ ; rm -r SICAPv2/", shell=True)
        ds = SicapDataset(root=root, transform=transform, split=split, **kwargs)
        ds.classes = default_classnames["sicap"]
    elif dataset_name == "roco":
        ds = RoCo(dataset_name, transform=transform, **kwargs)
    elif dataset_name == "mimic_cxr_single" or dataset_name == "mimic_cxr_double":
        ds = MIMIC_CXR(dataset_name, root=root, split=split, transform=transform, **kwargs)
    elif dataset_name == "mimic_cxr_chexpert" or dataset_name == "mimic_cxr_negbio":
        ds = MIMIC_CXR_LP(dataset_name, root=root, split=split, transform=transform, **kwargs)
        ds.classes = default_classnames["mimic_cxr"]
    elif dataset_name == "vindr_mammo":
        if not os.path.exists(root):
            # Automatic download
            print("Downloading vindr_mammo...")
            if not has_kaggle():
                print("Kaggle is needed to download the dataset. Please install it via `pip install kaggle`")
                sys.exit(1)
            call(f"mkdir {root}", shell=True)
            call(f"kaggle datasets download -d hadighahroudi/vindr-mammo-dataset -p {root}", shell=True)
            call(f"cd {root} ; unzip vindr-mammo-dataset.zip ; rm vindr-mammo-dataset.zip", shell=True)
        ds =  VinDr_Mammo(dataset_name, root=root, split=split, transform=transform, **kwargs)
        ds.classes = default_classnames["vindr_mammo"]
    elif dataset_name == "skin_cancer":
        if not os.path.exists(root):
            # Automatic download
            print("Downloading skin_cancer...")
            if not has_kaggle():
                print("Kaggle is needed to download the dataset. Please install it via `pip install kaggle`")
                sys.exit(1)
            call(f"kaggle datasets download -d farjanakabirsamanta/skin-cancer-dataset -p {root}", shell=True)
            call(f"cd {root} ; unzip skin-cancer-dataset.zip ; rm skin-cancer-dataset.zip", shell=True)
            call(f"cd {root} ; mv 'Skin Cancer/Skin Cancer' skin_cancer/ ; rm -r 'Skin Cancer'", shell=True)
        ds =  SkinCancer(dataset_name, root=root, split=split, transform=transform, **kwargs)
        ds.classes = default_classnames["skin_cancer"]
    elif dataset_name == "pad_ufes_20":
        if not os.path.exists(root):
            # Automatic download
            print("Downloading pad_ufes_20...")
            if not has_kaggle():
                print("Kaggle is needed to download the dataset. Please install it via `pip install kaggle`")
                sys.exit(1)
            call(f"mkdir {root}", shell=True)
            call(f"kaggle datasets download -d moeinmadadi/pad-ufes-20 -p {root}", shell=True)
            call(f"cd {root} ; unzip pad-ufes-20.zip ; rm pad-ufes-20.zip", shell=True)
            call(f"cd {root} ; mv PAD-UFES-20/* ./ ; rm -r PAD-UFES-20", shell=True)
        ds =  PadUfes20(dataset_name, root=root, split=split, transform=transform, **kwargs)
        ds.classes = default_classnames["pad_ufes_20"]
    elif dataset_name in ['pathmnist', 'chestmnist', 'dermamnist', 'octmnist', 'pneumoniamnist', 'retinamnist',
                          'breastmnist', 'bloodmnist', 'tissuemnist', 'organamnist', 'organcmnist', 'organsmnist']:
        ds = MedMNIST(dataset_name, root=root, split=split, transform=transform, **kwargs)
        ds.classes = default_classnames[dataset_name]
    elif dataset_name == "deepeyenet":
        ds = DeepEyeNet(dataset_name, root=root, split=split, transform=transform, **kwargs)
    elif dataset_name == "quilt_1m" or dataset_name == "quilt_1m_no_pubmed" or dataset_name == "quilt_1m_no_laion" or dataset_name == "quilt_1m_no_pubmed_no_laion":
        ds = Quilt1M(dataset_name, root=root, split=split, transform=transform, **kwargs)
    elif dataset_name in ['pathmnist_plus', 'chestmnist_plus', 'dermamnist_plus', 'octmnist_plus', 'pneumoniamnist_plus', 'retinamnist_plus',
                          'breastmnist_plus', 'bloodmnist_plus', 'tissuemnist_plus', 'organamnist_plus', 'organcmnist_plus', 'organsmnist_plus']:
        if not os.path.exists(root):
            # Automatic download
            print("Downloading MedMNIST+...")
            call(f"mkdir {root}", shell=True)
            call(f"curl 'https://zenodo.org/api/records/10519652/files-archive' --output {root}/data.zip", shell=True)
            call(f"cd {root} ; unzip data.zip *_224.npz ; rm data.zip", shell=True)
        ds = MedMNISTPlus(dataset_name, root=root, split=split, transform=transform, **kwargs)
        # classes are the same as medmnist datasets without '_plus'
        ds.classes = default_classnames[dataset_name[:-5]]
...
def get_dataset_default_task(dataset):
    if dataset in ("deepeyenet", "mimic_cxr_single", "mimic_cxr_double", "quilt_1m", "roco", "flickr30k", "flickr8k", "mscoco_captions", "multilingual_mscoco_captions", "flickr30k-200", "crossmodal3600", "xtd200"):
...
def get_dataset_collate_fn(dataset_name):
    if dataset_name in ("deepeyenet", "mimic_cxr_single", "mimic_cxr_double", "quilt_1m", "roco", "mscoco_captions", "multilingual_mscoco_captions", "flickr30k", "flickr8k", "flickr30k-200", "crossmodal3600", "xtd200", "winoground") or dataset_name.startswith("sugar_crepe"):
