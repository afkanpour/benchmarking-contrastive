...
from torchmetrics.functional.classification import multilabel_auroc, multilabel_f1_score, multilabel_accuracy, multiclass_f1_score, multiclass_auroc, multiclass_average_precision, multiclass_accuracy, multilabel_average_precision
...
def train(dataloader, input_shape, output_shape, weight_decay, lr, epochs, autocast, device, seed, multi_label=False):
    ...
    for epoch in range(epochs):
        ...
        for i, (x, y) in enumerate(dataloader):
            ...
            with autocast():
                pred = model(x)
                loss = criterion(pred, y.type(torch.float32) if multi_label else y)
...
def find_peak(wd_list, idxs, train_loader, val_loader, input_shape, output_shape, lr, epochs, autocast, device, verbose, seed, multi_label=False):
    best_wd_idx, max_acc = 0, 0
    for idx in idxs:
        weight_decay = wd_list[idx]
        model = train(train_loader, input_shape, output_shape, weight_decay, lr, epochs, autocast, device, seed, multi_label)
...
def evaluate(model, train_dataloader, dataloader, fewshot_k, batch_size, num_workers, lr, epochs, 
             model_id, seed, feature_root, device, val_dataloader=None, normalize=True, amp=True, verbose=False, multi_label=False):
    ...
    featurizer = Featurizer(model, normalize)
    if torch.cuda.is_available():
    ...
    if val_dataloader is not None:
        ...
        peak_idx = find_peak(wd_list, wd_init_idx, feature_train_loader, feature_val_loader, input_shape, output_shape, lr, epochs, autocast, device, verbose, seed, multi_label)
        step_span = 8
        while step_span > 0:
            left, right = max(peak_idx - step_span, 0), min(peak_idx + step_span, len(wd_list)-1)
            peak_idx = find_peak(wd_list, [left, peak_idx, right], feature_train_loader, feature_val_loader, input_shape, output_shape, lr, epochs, autocast, device, verbose, seed, multi_label)
            step_span //= 2
    ...
    final_model = train(train_loader, input_shape, output_shape, best_wd, lr, epochs, autocast, device, seed, multi_label)
    ...
    if multi_label:
        ...
        logits = logits.float()
        target = target.int()
        map_macro = multilabel_average_precision(logits, target, num_labels=num_classes, average="macro", thresholds=None)
        map_weighted = multilabel_average_precision(logits, target, num_labels=num_classes, average="weighted", thresholds=None)
        if verbose:
            print(f"mAP (Macro): {map_macro}")
            print(f"mAP (Weighted): {map_weighted}")
        
        # compute accuracy
        acc_micro = multilabel_accuracy(logits, target, num_labels=num_classes, average="micro")
        acc_macro = multilabel_accuracy(logits, target, num_labels=num_classes, average="macro")
        acc_weighted = multilabel_accuracy(logits, target, num_labels=num_classes, average="weighted")
        if verbose:
            print(f"Accuracy (Micro): {acc_micro}")
            print(f"Accuracy (Macro): {acc_macro}")
            print(f"Accuracy (Weighted): {acc_weighted}")
        
        # compute f1-score
        f1_micro = multilabel_f1_score(logits, target, num_labels=num_classes, average="micro")
        f1_macro = multilabel_f1_score(logits, target, num_labels=num_classes, average="macro")
        f1_weighted = multilabel_f1_score(logits, target, num_labels=num_classes, average="weighted")
        if verbose:
        ...
        # compute auc-roc
        aucroc_macro = multilabel_auroc(logits, target, num_labels=num_classes, average="macro", thresholds=None)
        aucroc_weighted = multilabel_auroc(logits, target, num_labels=num_classes, average="weighted", thresholds=None)
        if verbose:
            print(f"AUC-ROC (Macro): {aucroc_macro}")
            print(f"AUC-ROC (Weighted): {aucroc_weighted}")

        return {'epochs': epochs, 'seed': seed, 'fewshot_k': fewshot_k, 'normalized': normalize,
                "map_macro": map_macro.item(), "map_weighted": map_weighted.item(),
                "acc_micro": acc_micro.item(), "acc_macro": acc_macro.item(), "acc_weighted": acc_weighted.item(),
                "f1_micro": f1_micro.item(), "f1_macro": f1_macro.item(), "f1_weighted": f1_weighted.item(),
                "aucroc_macro": aucroc_macro.item(), "aucroc_weighted": aucroc_weighted.item()}
        ...
        logits = logits.float()
        target = target.int()
        map_macro = multiclass_average_precision(logits, target, num_classes=num_classes, average="macro", thresholds=None)
        map_weighted = multiclass_average_precision(logits, target, num_classes=num_classes, average="weighted", thresholds=None)
        if verbose:
            print(f"mAP (Macro): {map_macro}")
            print(f"mAP (Weighted): {map_weighted}")
        
        # compute accuracy
        acc_micro = multiclass_accuracy(logits, target, num_classes=num_classes, average="micro")
        acc_macro = multiclass_accuracy(logits, target, num_classes=num_classes, average="macro")
        acc_weighted = multiclass_accuracy(logits, target, num_classes=num_classes, average="weighted")
        if verbose:
            print(f"Accuracy (Micro): {acc_micro}")
            print(f"Accuracy (Macro): {acc_macro}")
            print(f"Accuracy (Weighted): {acc_weighted}")
        ...
        # compute f1-score
        f1_micro = multiclass_f1_score(logits, target, num_classes=num_classes, average="micro")
        f1_macro = multiclass_f1_score(logits, target, num_classes=num_classes, average="macro")
        f1_weighted = multiclass_f1_score(logits, target, num_classes=num_classes, average="weighted")
        if verbose:
            print(f"F1-score (Micro): {f1_micro}")
            print(f"F1-score (Macro): {f1_macro}")
            print(f"F1-score (Weighted): {f1_weighted}")
        ...
        # compute auc-roc
        aucroc_macro = multiclass_auroc(logits, target, num_classes=num_classes, average="macro", thresholds=None)
        aucroc_weighted = multiclass_auroc(logits, target, num_classes=num_classes, average="weighted", thresholds=None)
        if verbose:
            print(f"AUC-ROC (Macro): {aucroc_macro}")
            print(f"AUC-ROC (Weighted): {aucroc_weighted}")
