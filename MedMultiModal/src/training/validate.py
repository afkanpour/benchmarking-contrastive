...
def main(args):
    ...
    # list all epochs
    checkpoint_files = [file for file in os.listdir(args.checkpoints_dir) if file.endswith(".pt")]
    checkpoint_files = sorted(checkpoint_files, key=lambda name: int(name[6:-3]))
    print("All checkpoint files to run a validation loop:\n", f"checkpoint directory: {args.checkpoints_dir}", "\n", "\n".join(checkpoint_files))

    # do the rest for each checkpoint
    for ckpt in checkpoint_files:
        args.pretrained = os.path.join(args.checkpoints_dir, ckpt)
