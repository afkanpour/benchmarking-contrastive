...
def main(args):
    ...
    if args.mask_contrast:
        model_kwargs["mask_contrast"] = args.mask_contrast
    if args.filip:
        model_kwargs["filip"] = True
    model_kwargs["caption_encoder"] = args.caption_encoder
    ...
    if args.force_patch_dropout:
        model.visual.patch_dropout = PatchDropout(args.force_patch_dropout)
    if args.distill:
    ...
    if args.lock_text_decoder:
        model.lock_decoder_tower()
    # print training status of the model
    ...
    if args.train_data or args.dataset_type == "synthetic":
        ...
        optimizer_grouped_parameters = [
            ...
            {"params": rest_params, "weight_decay": args.wd, "name": "rest"},
        ]
    ...
    if "train" not in data:
        ...
        # Evaluate.
        evaluate(model, data, start_epoch, args, tb_writer=writer, tokenizer=tokenizer, loss=loss)
        return None

    if args.pretrained and any(
        v in data for v in ("val", "imagenet-val", "imagenet-v2", "pathmnist")
    ):
        evaluate(model, data, 0, args, tb_writer=writer, tokenizer=tokenizer, loss=loss)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if any(v in data for v in ("val", "imagenet-val", "imagenet-v2", "pathmnist")):
            evaluate(
                model,
                data,
                completed_epoch,
