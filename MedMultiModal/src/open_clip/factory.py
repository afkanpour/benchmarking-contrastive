...
def create_model(
    ...
    **model_kwargs,
    ...
    caption_encoder = model_kwargs.pop("caption_encoder", None)
    ...
    if pretrained and pretrained.lower() == "openai":
        ...
        model = load_openai_model(
            model_name.replace("-FILIP", "") if "FILIP" in model_name else model_name,
            precision=precision,
            device=device,
            cache_dir=cache_dir,
        )
        ...
        if "Shared" in model_name:
            model.transformer = model.visual.transformer
            num_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
            print("Number of parameters with shared encoder: ", num_params)
        ...
        if pretrained:
            ...
            if checkpoint_path and "coca" not in model_name.lower():
                logging.info(f"Loading pretrained {model_name} weights ({pretrained}).")
                load_checkpoint(model, checkpoint_path)
            elif "coca" not in model_name.lower():
                error_str = (
        ...
        if "coca" in model_name.lower() and caption_encoder == "ViT-B-16":
            clip_model = load_openai_model(
                caption_encoder,
                precision=precision,
                device=device,
                cache_dir=cache_dir,
            )
            clip_model.visual.attn_pool = model.visual.attn_pool
            clip_model.visual.attn_pool_type = model.visual.attn_pool_type
            clip_model.visual.attn_pool_contrastive = model.visual.attn_pool_contrastive
            clip_model.visual.ln_post = model.visual.ln_post
            clip_model.visual.pool_type = model.visual.pool_type
            clip_model.visual.proj = model.visual.proj
            clip_model.visual.output_tokens = model.visual.output_tokens
            model.visual = clip_model.visual

            model.text.transformer = clip_model.transformer
            model.text.token_embedding = clip_model.token_embedding
            model.text.positional_embedding = clip_model.positional_embedding
            model.text.ln_final = clip_model.ln_final
            model.text.text_projection = clip_model.text_projection
            model.text.attn_mask = clip_model.attn_mask
            model.openai_transformer = True
    ...
    if "mask_contrast" in model_kwargs:
        model.visual.mask_contrast = model_kwargs["mask_contrast"]
...
def create_loss(args):
    ...
    elif args.image_contrast:
        return ContrastClipLoss(
            local_loss=args.local_loss,
            gather_with_grad=args.gather_with_grad,
            cache_labels=True,
            rank=args.rank,
